#include "thermal_control/external_loop.hpp"
...
{
  internal_sub_ = this->create_subscription<thermal_control::msg::InternalLoopStatus>(
    "/tcs/internal_loop_heat", 10,
    std::bind(&ExternalLoopA::interface_heat_exchanger, this, _1));

  ammonia_client_ = this->create_client<thermal_control::srv::CoolantFlow>("/tcs/request_ammonia");

  loop_status_pub_ = this->create_publisher<thermal_control::msg::ExternalLoopStatus>(
...
void ExternalLoopA::try_ammonia_refill()
{
  ...
  auto req = std::make_shared<thermal_control::srv::CoolantFlow::Request>();
  req->requested_volume = 5.0;

  awaiting_ammonia_response_ = true;

  auto future = ammonia_client_->async_send_request(req,
    [this](rclcpp::Client<thermal_control::srv::CoolantFlow>::SharedFuture result) {
      awaiting_ammonia_response_ = false;
...
void ExternalLoopA::interface_heat_exchanger(const thermal_control::msg::InternalLoopStatus::SharedPtr msg)
{
  if (!ammonia_filled_) {
  ...
  thermal_control::msg::ExternalLoopStatus status;
...
int main(int argc, char **argv)
{
  ...
  rclcpp::spin(std::make_shared<thermal_control::ExternalLoopA>());
  rclcpp::shutdown();
  return 0;
}
