#include "thermal_control/thermal_solver.hpp"
...
: Node("thermal_solver_node")
{
  ...
  node_pub_ = this->create_publisher<thermal_control::msg::ThermalNodeDataArray>(
    "/thermal/nodes/state", 10);
  link_pub_ = this->create_publisher<thermal_control::msg::ThermalLinkFlowsArray>(
    "/thermal/links/flux", 10);
  cooling_client_ = this->create_client<thermal_control::srv::NodeHeatFlow>(
...
void ThermalSolverNode::updateSimulation()
{
  ...
  thermal_control::msg::ThermalNodeDataArray node_msg;
  for (const auto &[name, node] : thermal_nodes_) {
    thermal_control::msg::ThermalNodeData data;
    data.name = name;
  ...
  thermal_control::msg::ThermalLinkFlowsArray link_msg;
  for (const auto &link : thermal_links_) {
    thermal_control::msg::ThermalLinkFlows l;
    l.node_a = link.from;
    l.node_b = link.to;
