#include "thermal_control/on_demand_publisher.hpp"

using std::placeholders::_1;
using std::placeholders::_2;


using namespace thermal_control;
FocusNodeMonitor::FocusNodeMonitor()
: rclcpp::Node("focus_node_monitor")
{
  node_sub_ = this->create_subscription<thermal_control::msg::ThermalNodeDataArray>(
    "/thermal/nodes/state", 10,
    std::bind(&FocusNodeMonitor::thermal_node_callback, this, _1));

  service_ = this->create_service<thermal_control::srv::GetSubTopic>(
    "/thermal/focus_node",
...
void FocusNodeMonitor::thermal_node_callback(
  const thermal_control::msg::ThermalNodeDataArray::SharedPtr msg)
{
  latest_nodes_.clear();
  ...
  std::shared_ptr<thermal_control::srv::GetSubTopic::Response> response)
{
  ...
  if (focused_publishers_.find(name) == focused_publishers_.end()) {
    ...
    focused_publishers_[name] = this->create_publisher<thermal_control::msg::ThermalNodeData>(topic, 10);
...
int main(int argc, char **argv)
{
  ...
  rclcpp::spin(std::make_shared<thermal_control::FocusNodeMonitor>());
  rclcpp::shutdown();
  return 0;
}
